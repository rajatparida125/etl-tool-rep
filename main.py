...
with col1:
    raw_file = st.file_uploader("📁 Upload raw CSV/Excel", type=['csv', 'xlsx', 'xls'])
with col2:
    mapping_file = st.file_uploader("🗺️ Upload mapping file (optional)", type=['csv', 'xlsx', 'xls'])
...
if raw_file is not None:
    ...
    # Step 2: Configure transforms (only show if raw data loaded)
    st.subheader("⚙️ Configure Output Columns")
    
    # Column configuration
    st.markdown("**For each output column:**")
    
    # Simple column builder: rename + mapping
    output_columns = []
    
    num_output_cols = st.number_input("Number of output columns", min_value=1, max_value=10, value=3)
    
    for i in range(num_output_cols):
        with st.expander(f"Output Column {i+1}"):
            col_name = st.text_input(f"Output column name", value=f"col_{i+1}")
            
            # Source column selection
            source_col = st.selectbox(f"Source column", options=['None'] + list(df.columns.tolist()))
            
            # Mapping (if mapping file exists)
            mapping_choice = "None"
            if mapping_df is not None:
                mapping_choice = st.selectbox("Apply mapping?", 
                                            options=["None"] + mapping_df.columns.tolist())
        ...
        for config in output_columns:
            if config['source'] != 'None':
                ...
                if config['mapping_key'] and mapping_df is not None:
                    map_dict = mapping_df.set_index(config['mapping_key']).to_dict()
                    if 'target' in map_dict:  # Assume target column exists
                        result_df[config['name']] = result_df[config['name']].map(map_dict['target']).fillna(result_df[config['name']])
                    st.info(f"✅ Mapped {config['name']} using {config['mapping_key']} → target")
